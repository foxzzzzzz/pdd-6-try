...
- Browser launch now keeps Chrome sandbox enabled by default and no longer passes a fixed `--window-size`; patrol flow detects PDD slider/captcha/security challenges, writes screenshot/HTML/diagnostic evidence, pauses the current store, stops the current patrol, and leaves visible Chrome open for manual handling.
- Browser contexts no longer force Playwright `viewport=1920x1080`; visible Chrome now uses the real maximized window dimensions to reduce the inner/outer/screen fingerprint mismatch seen in PDD risk checks.
- Login-state checks now run the PDD security-challenge guard before classifying a page as `login-required`, so slider verification on `/login/` is captured as `security-challenge-login-page-*` instead of only a login-required screenshot.
- Browser language preferences now align automation Chrome with manual Chrome by setting `Accept-Language: zh-CN,zh;q=0.9` and persistent profile `intl.accept_languages=zh-CN,zh`.
- Login-page security challenge detection now runs again after the page is classified as `login_required`, covering slider dialogs that appear asynchronously after the first login-page check.
- Screenshot-based security challenge detection now recognizes local slider-dialog panels even when a wide white area prevents the global dim-overlay heuristic from activating.
- Login-state checks now also re-check for security challenges after an authenticated home page is detected, preventing login-bind success from closing Chrome before an async homepage slider dialog is caught.
- Experiment D1 reduces the automation Chrome launch signature by asking Playwright to skip several nonessential default flags (`--disable-background-networking`, `--disable-component-update`, `--disable-default-apps`, `--disable-extensions`, `--disable-sync`, `--password-store=basic`, `--use-mock-keychain`) while keeping the same profile, viewport, language, and control transport for isolated PDD slider testing.
- Browser profile locking now treats `.profile.lock` files whose recorded owner PID is no longer running as stale immediately, preventing a closed automation Chrome from blocking the next patrol for the full stale-lock timeout.
- Security-challenge diagnostics now include live Windows `chrome.exe` process snapshots with PID, parent PID, full command line, and active-profile matching so launch flags can be verified from the saved JSON without racing the browser window.
- Experiment E0 removes Playwright context `locale` so Chrome profile language preferences can drive `navigator.languages`, and clears PDD per-host zoom from the worker profile so `/home/` no longer inherits the stale `mms.pinduoduo.com` zoom that made `innerWidth` exceed `outerWidth`.
- Experiment E1 adds an opt-in `BROWSER_LAUNCH_MODE=external-cdp` path: the worker starts visible system Chrome with only `--remote-debugging-port`, `--user-data-dir`, and `about:blank`, then connects through CDP to test whether PDD's slider is tied to Playwright's normal launch signature.
- Screenshot security-challenge fallback now avoids treating normal backend chart tooltips and dashboard cards as slider verification dialogs; the detector requires a clearer light-gray slider track and stronger white handle signal before pausing a patrol.
- Screenshot security-challenge fallback now avoids treating normal service-data cards on `/sycm/goods_quality/pilot_mall` as slider verification dialogs; the detector requires a stronger slider-track signal and real false-positive screenshots now classify as normal pages.
- Patrol navigation now checks for visible captcha/slider/puzzle challenge elements immediately after each page load, so security verification popups pause the run before collectors continue parsing the blocked page.
- Security challenge detection now scans all frames and shadow DOM for visible challenge elements, and adds a screenshot-pixel fallback for globally dimmed pages with centered or off-center slider dialogs.
